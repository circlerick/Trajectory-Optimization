...
static void w_binary_expand_op(real_T in1_data[], const int32_T in1_size[2],
                               int32_T in2, real_T in3, const real_T in4_data[],
                               ...
                               const int32_T *in4_size)
{
  ...
  for (i = 0; i < loop_ub; i++) {
    b_in1_data[i] = in1_data[i * stride_0_0 + in1_size[0] * in2] +
                    in3 * in4_data[i * stride_1_0];
  }
  for (i = 0; i < b_in1_size; i++) {
    in1_data[i + in1_size[0] * in2] = b_in1_data[i];
  }
}
              ...
              int32_T yinterp_size[2])
{
  ...
  int32_T i;
  int32_T k;
  yinterp_size[0] = y_size;
  yinterp_size[1] = 7;
  for (i = 0; i < 7; i++) {
    for (b_i = 0; b_i < y_size; b_i++) {
      yinterp_data[b_i + y_size * i] = y_data[b_i];
    }
  }
  for (k = 0; k < 7; k++) {
    real_T sp_data[243];
    ...
    b_b[0] += theta;
    k_mtimes(f_data, f_size, b_b, sp_data, &b_i);
    loop_ub = yinterp_size[0];
    if (yinterp_size[0] == b_i) {
      for (i = 0; i < loop_ub; i++) {
        sp_data[i] = yinterp_data[i + yinterp_size[0] * k] + h * sp_data[i];
      }
      for (i = 0; i < loop_ub; i++) {
        yinterp_data[i + yinterp_size[0] * k] = sp_data[i];
      }
    } else {
      w_binary_expand_op(yinterp_data, yinterp_size, k, h, sp_data, &b_i);
    }
  }
}
            ...
            int32_T ypinterp_size[2])
{
  ...
  for (k = 0; k < 13; k++) {
    ...
    b_b = theta * theta;
    for (b_i = 0; b_i < 14; b_i++) {
      b[b_i] = ((((((dv1[b_i] * theta + dv2[b_i]) * theta + dv3[b_i]) * theta +
                   dv4[b_i]) *
                      theta +
                  dv5[b_i]) *
                     theta +
    ...
    b[0] += theta;
    k_mtimes(f_data, f_size, b, sp_data, &b_i);
    loop_ub = yinterp_size[0];
    if (yinterp_size[0] == b_i) {
      for (i = 0; i < loop_ub; i++) {
        sp_data[i] = yinterp_data[i + yinterp_size[0] * k] + h * sp_data[i];
  ...
  ypinterp_size[0] = (uint8_T)yinterp_size[0];
  ypinterp_size[1] = 13;
  loop_ub = (uint8_T)yinterp_size[0] * 13;
  if (loop_ub - 1 >= 0) {
    memset(&ypinterp_data[0], 0, (uint32_T)loop_ub * sizeof(real_T));
  }
  for (k = 0; k < 13; k++) {
    theta = (tinterp[k] - t) / h;
    for (b_i = 0; b_i < 14; b_i++) {
      b[b_i] = ((((((a[b_i] * theta + y[b_i]) * theta + b_y[b_i]) * theta +
                   c_y[b_i]) *
                      theta +
                  d_y[b_i]) *
                     theta +
    ...
    k_mtimes(f_data, f_size, b, sp_data, &b_i);
    for (i = 0; i < b_i; i++) {
      ypinterp_data[i + ypinterp_size[0] * k] = sp_data[i];
    }
  }
}
