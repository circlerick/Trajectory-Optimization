           ...
           emxArray_real_T *varargout_1, emxArray_real_T *varargout_2)
{
  ...
  emxInit_real_T(&b_st, &f0, 1, &ub_emlrtRTEI);
  ...
  if ((uint32_T)b_y0->size[0] == 0U) {
    emlrtDivisionByZeroErrorR2012b(&ge_emlrtRTEI, &st);
  } else {
    i = (int32_T)(8192U / (uint32_T)b_y0->size[0]);
  }
  i += 8;
  chunk = muIntScalarMin_sint32(400, i);
  emxInit_real_T(&st, &tout, 2, &ib_emlrtRTEI);
  i = tout->size[0] * tout->size[1];
  tout->size[0] = 1;
  tout->size[1] = chunk;
  emxEnsureCapacity_real_T(&st, tout, i, &ib_emlrtRTEI);
  tout_data = tout->data;
  for (i = 0; i < chunk; i++) {
    tout_data[i] = 0.0;
  }
  emxInit_real_T(&st, &yout, 2, &jb_emlrtRTEI);
  i = yout->size[0] * yout->size[1];
  yout->size[0] = b_y0->size[0];
  yout->size[1] = chunk;
  emxEnsureCapacity_real_T(&st, yout, i, &jb_emlrtRTEI);
  yout_data = yout->data;
  ...
  emxInit_real_T(&st, &y, 1, &lb_emlrtRTEI);
  ...
  emxEnsureCapacity_real_T(&st, y, i, &lb_emlrtRTEI);
  y_data = y->data;
  nrows = b_y0->size[0];
  for (i = 0; i < nrows; i++) {
    y_data[i] = y0_data[i];
  }
  emxInit_real_T(&st, &f, 2, &mb_emlrtRTEI);
  i = f->size[0] * f->size[1];
  f->size[0] = b_y0->size[0];
  f->size[1] = 14;
  emxEnsureCapacity_real_T(&st, f, i, &mb_emlrtRTEI);
  f_data = f->data;
  ...
  MinStepExit = false;
  Done = false;
  emxInit_real_T(&st, &ynew, 1, &nb_emlrtRTEI);
  emxInit_real_T(&st, &a__2, 2, &vb_emlrtRTEI);
  emxInit_real_T(&st, &youtnew, 2, &rb_emlrtRTEI);
  emxInit_real_T(&st, &fE, 1, &vb_emlrtRTEI);
  emxInit_real_T(&st, &te, 2, &vb_emlrtRTEI);
  emxInit_real_T(&st, &ye, 2, &vb_emlrtRTEI);
  emxInit_int32_T(&st, &ie, &vb_emlrtRTEI);
  emxInit_real_T(&st, &r, 2, &vb_emlrtRTEI);
  emxInit_real_T(&st, &varargin_1, 2, &wb_emlrtRTEI);
  real_T d;
  real_T h;
  int32_T exitg1;
  do {
    ...
    do {
      ...
      if (y->size[0] == ynew->size[0]) {
        ...
        emxEnsureCapacity_real_T(&st, ynew, i, &nb_emlrtRTEI);
        ynew_data = ynew->data;
        nrows = y->size[0];
        for (i = 0; i < nrows; i++) {
    ...
    if (MinStepExit) {
      ...
      if ((ncols > 0) && empty_non_axis_sizes) {
        ...
        i = ynew->size[0];
        ynew->size[0] = neq;
        emxEnsureCapacity_real_T(&st, ynew, i, &pb_emlrtRTEI);
        ynew_data = ynew->data;
        for (i = 0; i < neq; i++) {
      ...
      emxEnsureCapacity_real_T(&c_st, youtnew, i, &rb_emlrtRTEI);
      ...
      if (nout + 1 > tout->size[1]) {
        ...
        tout->size[0] = 1;
        tout->size[1] += chunk;
        emxEnsureCapacity_real_T(&b_st, tout, i, &sb_emlrtRTEI);
        tout_data = tout->data;
        ...
        yout->size[1] += chunk;
        emxEnsureCapacity_real_T(&b_st, yout, b_i, &sb_emlrtRTEI);
        yout_data = yout->data;
        c_st.site = &xh_emlrtRSI;
      ...
      if (Done) {
        ...
        emxEnsureCapacity_real_T(&st, y, i, &tb_emlrtRTEI);
        y_data = y->data;
        nrows = ynew->size[0];
        for (i = 0; i < nrows; i++) {
  ...
  emxEnsureCapacity_real_T(&st, varargout_1, i, &ob_emlrtRTEI);
  ...
  emxEnsureCapacity_real_T(&st, varargout_2, i, &qb_emlrtRTEI);
