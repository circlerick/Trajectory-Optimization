             ...
             emxArray_real_T *varargout_1, emxArray_real_T *varargout_2)
{
  ...
  if (tspan[0] == tspan[1]) {
    emlrtErrorWithMessageIdR2018a(
        &st, &l_emlrtRTEI, "MATLAB:odearguments:TspanEndpointsNotDistinct",
  ...
  emxInit_real_T(&b_st, &f0, 1, &dd_emlrtRTEI);
  ...
  if (f0->size[0] != b_y0->size[0]) {
    emlrtErrorWithMessageIdR2018a(&st, &j_emlrtRTEI, "Coder:toolbox:SizeIC",
  ...
  if ((uint32_T)b_y0->size[0] == 0U) {
    emlrtDivisionByZeroErrorR2012b(&eg_emlrtRTEI, &st);
  } else {
    i = (int32_T)(8192U / (uint32_T)b_y0->size[0]);
  }
  i += 8;
  chunk = muIntScalarMin_sint32(400, i);
  emxInit_real_T(&st, &tout, 2, &qc_emlrtRTEI);
  i = tout->size[0] * tout->size[1];
  tout->size[0] = 1;
  tout->size[1] = chunk;
  emxEnsureCapacity_real_T(&st, tout, i, &qc_emlrtRTEI);
  tout_data = tout->data;
  for (i = 0; i < chunk; i++) {
    tout_data[i] = 0.0;
  }
  emxInit_real_T(&st, &yout, 2, &rc_emlrtRTEI);
  i = yout->size[0] * yout->size[1];
  yout->size[0] = b_y0->size[0];
  yout->size[1] = chunk;
  emxEnsureCapacity_real_T(&st, yout, i, &rc_emlrtRTEI);
  yout_data = yout->data;
  ...
  emxInit_real_T(&st, &y, 1, &tc_emlrtRTEI);
  ...
  if (f0->size[0] == y->size[0]) {
    i = y->size[0];
    y->size[0] = f0->size[0];
    emxEnsureCapacity_real_T(&st, y, i, &sc_emlrtRTEI);
    y_data = y->data;
    nrows = f0->size[0];
    for (i = 0; i < nrows; i++) {
      y_data[i] = f0_data[i] / y_data[i];
    }
    rh = c_norm(y) / 0.028750509310437024;
  } else {
    b_st.site = &gc_emlrtRSI;
    rh = j_binary_expand_op(&b_st, f0, y);
  }
  if (absh * rh > 1.0) {
    absh = 1.0 / rh;
  }
  ...
  emxEnsureCapacity_real_T(&st, y, i, &tc_emlrtRTEI);
  y_data = y->data;
  nrows = b_y0->size[0];
  for (i = 0; i < nrows; i++) {
    y_data[i] = y0_data[i];
  }
  emxInit_real_T(&st, &f, 2, &uc_emlrtRTEI);
  i = f->size[0] * f->size[1];
  f->size[0] = b_y0->size[0];
  f->size[1] = 14;
  emxEnsureCapacity_real_T(&st, f, i, &uc_emlrtRTEI);
  f_data = f->data;
  ...
  MinStepExit = false;
  Done = false;
  emxInit_real_T(&st, &ynew, 1, &vc_emlrtRTEI);
  emxInit_real_T(&st, &a__2, 2, &ed_emlrtRTEI);
  emxInit_real_T(&st, &youtnew, 2, &ad_emlrtRTEI);
  emxInit_real_T(&st, &fE, 1, &ed_emlrtRTEI);
  emxInit_real_T(&st, &te, 2, &ed_emlrtRTEI);
  emxInit_real_T(&st, &ye, 2, &ed_emlrtRTEI);
  emxInit_int32_T(&st, &ie, &ed_emlrtRTEI);
  emxInit_real_T(&st, &r, 2, &ed_emlrtRTEI);
  emxInit_real_T(&st, &varargin_1, 2, &fd_emlrtRTEI);
  real_T d;
  real_T h;
  int32_T exitg1;
  do {
    ...
    do {
      ...
      if (y->size[0] == ynew->size[0]) {
        ...
        emxEnsureCapacity_real_T(&st, ynew, i, &vc_emlrtRTEI);
        ynew_data = ynew->data;
        nrows = y->size[0];
        for (i = 0; i < nrows; i++) {
        ...
        d_binary_expand_op(&b_st, ynew, y, h);
        ynew_data = ynew->data;
      }
      tnew = t + h;
      if (Done) {
        ...
        if (absh <= hmin) {
          ...
          if (NoFailedAttempts) {
            ...
            absh = muDoubleScalarMax(
                hmin,
                absh * muDoubleScalarMax(
                           0.1, 0.8 * muDoubleScalarPower(1.0E-13 / err,
                                                          0.1111111111111111)));
          } else {
    ...
    if (MinStepExit) {
      ...
      if ((ncols > 0) && empty_non_axis_sizes) {
        ...
        i = ynew->size[0];
        ynew->size[0] = neq;
        emxEnsureCapacity_real_T(&st, ynew, i, &xc_emlrtRTEI);
        ynew_data = ynew->data;
        for (i = 0; i < neq; i++) {
      ...
      emxEnsureCapacity_real_T(&c_st, youtnew, i, &ad_emlrtRTEI);
      ...
      if (nout + 1 > tout->size[1]) {
        ...
        tout->size[0] = 1;
        tout->size[1] += chunk;
        emxEnsureCapacity_real_T(&b_st, tout, i, &bd_emlrtRTEI);
        tout_data = tout->data;
        ...
        yout->size[1] += chunk;
        emxEnsureCapacity_real_T(&b_st, yout, b_i, &bd_emlrtRTEI);
        yout_data = yout->data;
        c_st.site = &gi_emlrtRSI;
      ...
      if (Done) {
        ...
        emxEnsureCapacity_real_T(&st, y, i, &cd_emlrtRTEI);
        y_data = y->data;
        nrows = ynew->size[0];
        for (i = 0; i < nrows; i++) {
  ...
  emxEnsureCapacity_real_T(&st, varargout_1, i, &wc_emlrtRTEI);
  ...
  emxEnsureCapacity_real_T(&st, varargout_2, i, &yc_emlrtRTEI);
           ...
           emxArray_real_T *varargout_1, emxArray_real_T *varargout_2)
{
  ...
  if (f0->size[0] == y->size[0]) {
    ...
    b_st.site = &gc_emlrtRSI;
    rh = e_binary_expand_op(&b_st, f0, y);
  }
  if (absh * rh > 1.0) {
    absh = 1.0 / rh;
  }
