...
static void f_binary_expand_op(real_T in1[2], const struct0_T *in2,
                               const real_T in3[111], int32_T in4);
...
real_T stopfuel(const emlrtStack *sp, const real_T s[111])
{
  real_T x[2];
  real_T EPS;
  real_T absxk;
  ...
  if (physical.EPS > 0.0) {
    ...
    if ((s[110] < -EPS) || (s[110] > EPS)) {
      value = EPS - muDoubleScalarAbs(s[110]);
    }
  } else {
    value = s[110];
  }
  ...
  if (EPS < 1.0) {
    ...
    if (EPS != (int32_T)EPS) {
      emlrtIntegerCheckR2012b(EPS, &nb_emlrtDCI, (emlrtConstCTX)sp);
    }
    if (((int32_T)EPS < 1) || ((int32_T)EPS > 111)) {
      emlrtDynamicBoundsCheckR2012b((int32_T)EPS, 1, 111, &yd_emlrtBCI,
                                    (emlrtConstCTX)sp);
    }
    i = (int32_T)EPS;
  }
  if ((i != 2) && (i != 1)) {
    emlrtDimSizeImpxCheckR2021b(2, i, &db_emlrtECI, (emlrtConstCTX)sp);
  }
  if (EPS < 1.0) {
    i1 = 0;
  } else {
  ...
  if ((i1 != 2) && (i1 != 1)) {
    emlrtDimSizeImpxCheckR2021b(2, i1, &eb_emlrtECI, (emlrtConstCTX)sp);
  }
  if (i == 2) {
