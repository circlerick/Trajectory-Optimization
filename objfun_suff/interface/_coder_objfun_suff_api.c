...
static real_T (*j_emlrt_marshallIn(const emlrtStack *sp, const mxArray *STM0,
                                   ...
                                   const char_T *identifier))[100]
{
  ...
  thisId.fParent = NULL;
  thisId.bParentIsCell = false;
  y = k_emlrt_marshallIn(sp, emlrtAlias(STM0), &thisId);
  emlrtDestroyArray(&STM0);
  return y;
}
...
void MEXGlobalSyncOutFunction(boolean_T skipDirtyCheck)
{
  ...
  if (skipDirtyCheck || (physical_dirty != 0U)) {
    emlrtPutGlobalVariable("physical", emlrt_marshallOut(&physical));
  }
}
                     ...
                     int32_T nlhs, const mxArray *plhs[2])
{
  ...
  STM0 = j_emlrt_marshallIn(&st, emlrtAlias(prhs[2]), "STM0");
  c_emlrt_marshallIn(&st, emlrtAliasP(prhs[3]), "physical", &b_physical);
  l_emlrt_marshallIn(&st, emlrtAliasP(prhs[4]), "boundary", &boundary);
  /* Marshall in global variables */
  MEXGlobalSyncInFunction(&st);
  /* Invoke the target function */
  emxInit_real_T(&st, &states, 2, &pc_emlrtRTEI);
  emxInit_real_T(&st, &times, 1, &pc_emlrtRTEI);
  objfun_suff(SD, &st, *lam0, *X0, *STM0, &b_physical, &boundary, states,
              times);
  /* Marshall out global variables */
  MEXGlobalSyncOutFunction(true);
  /* Marshall function outputs */
  states->canFreeData = false;
  plhs[0] = b_emlrt_marshallOut(states);
  emxFree_real_T(&st, &states);
  if (nlhs > 1) {
    times->canFreeData = false;
    plhs[1] = c_emlrt_marshallOut(times);
  }
  emxFree_real_T(&st, &times);
  emlrtHeapReferenceStackLeaveFcnR2012b(&st);
}

/* End of code generation (_coder_objfun_suff_api.c) */
