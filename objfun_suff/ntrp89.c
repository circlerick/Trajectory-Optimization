            ...
            const real_T f[1554], real_T yinterp[1443], real_T ypinterp[1443])
{
  ...
  for (k = 0; k < 13; k++) {
    ...
    for (b_i = 0; b_i < 14; b_i++) {
      b[b_i] = ((((((dv6[b_i] * theta + dv5[b_i]) * theta + dv4[b_i]) * theta +
                   dv3[b_i]) *
                      theta +
                  dv2[b_i]) *
                     theta +
  ...
  for (k = 0; k < 13; k++) {
    theta = (tinterp[k] - t) / h;
    for (b_i = 0; b_i < 14; b_i++) {
      b[b_i] = ((((((d_b[b_i] * theta + b_y[b_i]) * theta + c_y[b_i]) * theta +
                   d_y[b_i]) *
                      theta +
                  e_y[b_i]) *
                     theta +
    ...
    b[0]++;
    c_mtimes(f, b, *(real_T(*)[111]) & ypinterp[111 * k]);
  }
}
                ...
                const real_T BI7[14], const real_T BI8[14], real_T sp[111])
{
  ...
  for (i = 0; i < 14; i++) {
    b_b[i] =
        ((((((BI8[i] * theta + BI7[i]) * theta + BI6[i]) * theta + BI5[i]) *
               theta +
           BI4[i]) *
              theta +
          BI3[i]) *
