...
void odezero(const emlrtStack *sp, real_T v, real_T t, const real_T y[111],
             real_T tnew, const real_T ynew[111], real_T t0, real_T h,
             const real_T f[1554], int32_T *nout, emxArray_real_T *tout,
             ...
             boolean_T *stop)
{
  ...
  *vnew = stopfuel(&b_st, ynew);
  tR = tnew;
  memcpy(&yR[0], &ynew[0], 111U * sizeof(real_T));
  vR = *vnew;
  vtry = 0.0;
  ttry = tnew;
  lastmoved = 'N';
    ...
    if (d != muDoubleScalarSign(vR)) {
      ...
      if (0.0 * absx >= 0.0) {
        ...
        if (tol_tmp <= tol) {
          ...
          tout->size[0] = 1;
          tout->size[1] = 1;
          emxEnsureCapacity_real_T(sp, tout, exponent, &lc_emlrtRTEI);
          tout_data = tout->data;
          exponent = yout->size[0] * yout->size[1];
          yout->size[0] = 111;
          yout->size[1] = 1;
          emxEnsureCapacity_real_T(sp, yout, exponent, &hb_emlrtRTEI);
          yout_data = yout->data;
          exponent = iout->size[0] * iout->size[1];
          iout->size[0] = 1;
          iout->size[1] = 1;
          emxEnsureCapacity_int32_T(sp, iout, exponent, &mc_emlrtRTEI);
          iout_data = iout->data;
          for (i = 0; i < 111; i++) {
            yout_data[i] = 0.0;
          }
          tout_data[0] = tR;
          iout_data[0] = 1;
          for (exponent = 0; exponent < 111; exponent++) {
          ...
          *stop = (tL != t0);
          ...
          delta = absx * absx;
          for (i = 0; i < 14; i++) {
            b[i] = ((((((dv6[i] * absx + dv5[i]) * absx + dv4[i]) * absx +
                       dv3[i]) *
                          absx +
                      dv2[i]) *
                         absx +
          ...
          c_mtimes(f, b, ytry);
          for (exponent = 0; exponent < 111; exponent++) {
            ytry[exponent] = y[exponent] + h * ytry[exponent];
          }
          st.site = &th_emlrtRSI;
          b_st.site = &sg_emlrtRSI;
          ...
          if ((d != muDoubleScalarSign(vtry)) && (0.0 * (vtry - vL) >= 0.0)) {
            ...
            tR = ttry;
            ttry = absx;
            memcpy(&yR[0], &ytry[0], 111U * sizeof(real_T));
            absx = vR;
            vR = vtry;
            vtry = absx;
            if (lastmoved == 'R') {
  ...
  if (guard1 && (lastmoved != 'N')) {
    emlrtErrorWithMessageIdR2018a(sp, &o_emlrtRTEI, "MATLAB:odezero:LostEvent",
